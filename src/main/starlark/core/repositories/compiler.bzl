...
def _kotlin_compiler_impl(repository_ctx):
    ...
    # Generate the kotlin_repo_build.bzl file from template
    repository_ctx.template(
        "kotlin_repo_build.bzl",
        attr._repo_build_template,
        executable = False,
    )

    # Generate the BUILD.bazel file that loads and calls the macro
    repository_ctx.file(
        "BUILD.bazel",
        content = """# Generated by kotlin_compiler_git_repository
load("@bazel_skylib//:bzl_library.bzl", "bzl_library")
load(":kotlin_repo_build.bzl", "define_kotlin_repo")
...
kotlin_compiler_git_repository = repository_rule(
    ...
    attrs = {
        ...
        "_repo_build_template": attr.label(
            doc = "repository build macro template",
            default = ":kotlin_repo_build.bzl.tpl",
        ),
        "_template": attr.label(
            doc = "repository build file template (deprecated, kept for compatibility)",
